...
public class HookLoader implements IXposedHookLoadPackage {
     ...
     */
    private final String modulePackage = "org.fly.xposed_test";
    /**
    ...
    static {
        ...
        hostAppPackages.add("com.soft.apk008v");
    }
    ...
    public void handleLoadPackage(final XC_LoadPackage.LoadPackageParam loadPackageParam) throws Throwable {
        if (hostAppPackages.contains(loadPackageParam.packageName)) {
            ...
            new org.fly.xposed_test.app.System(loadPackageParam).hook();

    }

    /**
     * 安装app以后，系统会在/data/org.fly.xposed_test.app/下备份了一份.apk文件，通过动态加载这个apk文件，调用相应的方法
...
//        File apkFile = findApkFile(modulePackageName);
     ...
     * @param modulePackageName 当前模块包名
     * @return File 返回apk文件
     * @throws FileNotFoundException 在/data/org.fly.xposed_test.app/下的未找到本模块apk文件,请检查本模块包名配置是否正确.
    ...
    private File findApkFile(String modulePackageName) throws FileNotFoundException {
        ...
        if (apkFile == null) {
            throw new FileNotFoundException("没在/data/org.fly.xposed_test.app/下找到文件对应的apk文件");
        }
        return apkFile;
    }
    /**
    ...
    private File findApkFileAfterSDK21(String packageName) throws FileNotFoundException {
        File apkFile;
        File path = new File(String.format("/data/org.fly.xposed_test.app/%s-%s", packageName, "1"));
        if (!path.exists()) {
            path = new File(String.format("/data/org.fly.xposed_test.app/%s-%s", packageName, "2"));
        }
        if (!path.exists() || !path.isDirectory()) {
            throw new FileNotFoundException(String.format("没找到目录/data/org.fly.xposed_test.app/%s-%s", packageName, "1/2"));
        }
        apkFile = new File(path, "base.apk");
        if (!apkFile.exists() || apkFile.isDirectory()) {
            throw new FileNotFoundException(String.format("没找到文件/data/org.fly.xposed_test.app/%s-%s/base.apk", packageName, "1/2"));
        }
        return apkFile;
    }
    ...
    private File findApkFileBeforeSDK21(String packageName) throws FileNotFoundException {
        File apkFile = new File(String.format("/data/org.fly.xposed_test.app/%s-%s.apk", packageName, "1"));
        if (!apkFile.exists()) {
            apkFile = new File(String.format("/data/org.fly.xposed_test.app/%s-%s.apk", packageName, "2"));
        }
        if (!apkFile.exists() || apkFile.isDirectory()) {
            throw new FileNotFoundException(String.format("没找到文件/data/org.fly.xposed_test.app/%s-%s.apk", packageName, "1/2"));
        }
        return apkFile;
    }
}
